...
| 4.5 | 실기기 케이블 경로 검증 | 완료 | 사용자가 VB-CABLE을 설치한 뒤(2026-09-18) 케이블 재생측에 톤을 보내고 **ZH3 자신의 loopback**으로 관측: 엔진 OFF 0.000, 바이패스 1.00×(기준 대비), Circuit 0.69×, 양 채널·440 Hz·중복 없음. 검사기는 자기 판별력도 `--self-check`로 증명 |
| 4.5 | 30분 안정성·재시작 10회·3상태 비교 | 완료(한계 기록) | 30분 연속 3표본: **1차는 캡처 재동기 1회로 기준 초과(exit 1), 2·3차 통과**(프라이밍 후 underrun 증가 0). 정지/재시작 10/10, 3상태 OFF/바이패스/Circuit 비교 완료 — 수치·기전·미검증 항목은 [검증 문서](windows-routing-validation.md) |
