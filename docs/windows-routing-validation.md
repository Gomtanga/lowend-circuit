...
| **검사기 자신의 `channel_identity` 단계(실기기 판정)** | 실행 시점에 목적지(ZH3)가 다른 소스(사용자가 재생 중이던 동영상)로 소리를 내고 있었고, 새 `destination_idle` 가드가 그 상태를 잡아 **측정을 중단**했습니다. 채널 정체성 자체는 조용한 창에서 임시 프로브로 관찰했지만(3절), **커밋된 검사기가 그 단계의 판정을 스스로 낸 적은 아직 없습니다** | 목적지가 조용한 ~90초 창(톤 재생 동의 포함) |
...
3. **관찰하지 못한 항목**: 사람의 청취 평가, 물리 USB 탈착·Bluetooth 실행, 재개방 시점의 순환 충돌
   실기기 확인, 그리고 커밋된 검사기 자신의 `channel_identity` 단계 판정(목적지가 조용한 창이
   필요) — SKIPPED 표에 남아 있습니다.
