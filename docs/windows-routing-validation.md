...
| 코드 검증 대상 commit | `6d6eefa` — 이 커밋에서 Windows(3잡)·Core(4잡)·macOS CI가 모두 success입니다. `2d0517c`는 검사 스크립트(`scripts/check-windows-route-stability.py`)만 추가했습니다. **`6d6eefa` 이후의 C++ 변경은 네 개**입니다: `a06240f`(위생 — fixture id를 합성값으로 바꾸고 `AGENTS.md`에서 실제 경로를 지움, 동작 불변), `3f3badf`(순환 짝 판정을 컨테이너가 아니라 **장치 인스턴스**로 — 4절의 결함 행), `57a7001`(GUI 시작 안내가 **물리 출력**을 가리킴 — 4절의 결함 행), `f40d7de`(`--tone-channel`으로 한 채널씩 구동 — 3절의 채널 정체성 행). 동작 변경은 각각 회귀 검사와 주입 시험으로 확인했습니다. **마지막으로 CI를 확인한 commit은 `b1f054b`이고 거기서 16/16 check success**입니다(`8abb973`, `a93c1b1`, `c62acfc`, `e858876`도 16/16). 이 표가 지칭하는 CI는 항상 마지막 green 커밋이며, 그 뒤에 오는 문서 전용 커밋은 같은 워크플로가 검사합니다(확인: `git log --oneline 6d6eefa..HEAD -- Windows/Source`). PR #24가 아직 열려 있으므로 tip의 공식 상태는 후속 PR #25의 checks에서 확인합니다 |
| 이 표의 실기기 측정을 만든 바이너리 | 이 브랜치에서 빌드한 `build/win-cli/Release/lowend_windows.exe`(CLI 행)와 `build/win-cli/Release/lowend_gui.exe`(GUI 행), `build/win-routing-no-ui/Release/lowend_windows.exe`(no-UI 행), `build/win-cli/Debug/lowend_windows.exe`(Debug 행)입니다. 마지막 C++ 변경은 `f40d7de`(CLI `--tone-channel`, 소스 2026-09-18 16:29~16:30)이고, **세 구성 모두 그 뒤에 다시 빌드**했습니다: Release CLI 16:31:45(같은 시각 GUI 16:31:44), Debug CLI 16:51:26, no-UI Release 16:51:33 — 각각 소스보다 새것입니다. 이번 채널 검증 실행은 그 Release CLI 바이너리로 했고, `--self-test`도 **exit 0**(`All offline checks passed.`)입니다. "Release"라는 디렉터리 이름을 근거로 삼지 않습니다 |
...
| **채널 정체성 검사 (신규, 오프라인 판별력)** | `--tone-channel left\|right` + `check-windows-cable-route.py`의 `channel_identity` 단계 | 한 채널씩 구동하는 톤 옵션을 추가하고(거부 3종: 값 없음/잘못된 값/`--play-tone` 없이 — CLI 회귀 37건에 포함), 검사기가 좌·우 단독 구동을 각각 경로에 통과시켜 **구동한 쪽에 도착했는지**를 판정합니다. 판정은 **RMS**로 합니다(단일 표본이 판정을 좌우하지 않도록). `--self-check`가 교체된 단일 채널을 거부하고, 양쪽 채널을 단일 채널로 오판한 경우와 고립 표본 1개(0.14)를 각각 거부/수용합니다. **주입 시험 2건**: 판정이 구동 채널을 무시하도록 되돌리면 `a right-only signal was rejected when judged against the right side`로, bleed 한도를 무력화하면 `a two-channel signal passed the single-channel identity criterion`으로 self-check가 **exit 1**; 되돌린 뒤 exit 0 |
| **케이블 경로의 채널 정체성 (실기기, 임시 프로브)** | 엔진(bypass) 케이블 녹음 → ZH3, 톤은 케이블 재생측에 `--tone-channel left`, ZH3 자신의 loopback 관측, **3회** | **3회 모두 구동한 왼쪽에만 도착**(좌 peak 0.3073, 오른쪽은 0.005를 넘는 표본 **0개**). 같은 창에서 케이블 녹음측 직접 캡처도 왼쪽 0.4000/오른쪽 0.0000, ZH3에 직접 보낸 좌 단독 톤도 좌 0.3074/우 0.0000 — **케이블·DSP 경로·출력 endpoint 어디에서도 좌우 교체나 지속 누설이 관측되지 않았습니다**. 이 관측은 커밋된 검사기가 아니라 그때 쓴 임시 프로브(커밋하지 않았습니다)로 했습니다. 같은 조건은 `--tone-channel left`로 케이블 재생측에 톤을 보내고 `--monitor --dump-wav`로 ZH3 loopback을 기록해 되풀이할 수 있습니다. 그 뒤 **커밋된 검사기 자신의 `channel_identity` 단계도 목적지가 조용한 창에서 통과**했습니다: `left-only` 구동 채널 RMS **0.175580**, 미구동 채널 RMS **0.000303**(비 0.17%); `right-only` 구동 **0.175957**, 미구동 **0.000100**(비 0.06%). 미구동 채널의 peak는 **0.140961**이지만 그것은 시작 순간의 **표본 1개**이고, 판정은 RMS로 합니다(아래 결함 행) |
| **검사기의 `destination_idle` 사전 조건 (실기기)** | 같은 검사기 실행 | 검사기가 톤을 재생하기 **전에** 목적지 loopback을 관찰해 **peak 0.285278**을 보고 `FAILED`로 멈췄고, **톤을 한 번도 내보내지 않았습니다** — 다른 앱(당시 사용자가 재생 중이던 동영상)이 목적지로 재생 중이면 레벨 판정이 그 소스를 재는 것이 되므로 측정 자체를 거부합니다 |
| **채널 정체성 판정이 고립 표본을 누설로 오판 (자체 발견·수정)** | `channel_identity` 단계의 첫 실행들 | 구동하지 않은 채널이 **0.140961 / 0.067 / 0.021**로 나타났지만, 덤프를 5 ms 창으로 보면 **표본 1개**였고 나머지 구간은 0.0000이었습니다(구동 채널은 내내 0.3073). 좌우 교체가 아니라 톤이 시작되는 순간의 고립 표본이며, peak 기준은 그것을 "누설"로 판정합니다. 판정을 **RMS 비교**로 바꾸고 고립 표본 1개(0.14)를 수용하는 회귀를 `--self-check`에 추가했습니다(그 회귀는 peak 기준으로 되돌리면 실패합니다) |
| **케이블 경로 실행 검증 (이정표의 핵심)** | `python scripts\check-windows-cable-route.py build\win-cli\Release\lowend_windows.exe --cable-playback {…368e189b…}(CABLE Input) --cable-recording {…8eb85364…}(CABLE Output) --output {…275d2e95…}(ZH3)` | **exit 0 · "cable route verified"**. 톤(440 Hz stereo, 진폭 0.40, 3초)을 **케이블 재생측 endpoint로** 보내고 **ZH3 자신의 loopback**을 관측: 기준값(목적지 직접) peak 0.307343 (L/R 동일), 440.0 Hz · **엔진 OFF: peak 0.000000, 프레임 0**(우회·중복 경로 없음) · **바이패스**: peak 0.307343 = 기준의 1.00×, 440.0 Hz, 양 채널 · **Circuit**: peak 0.213287 = **바이패스의 0.69×**, 440.0 Hz, 양 채널. 즉 앱이 케이블에 재생한 신호가 케이블 녹음측 → LowEnd DSP → 지정한 물리 출력까지 실제로 도달하며, 두 채널·피치가 유지되고 원본/처리본이 겹치지 않습니다. 이 실행은 채널 정체성 단계까지 포함한 검사기 전체를 통과했습니다(`cable route verified`, exit 0). **이 검증은 디지털 경로 관측이며 사람의 청취 평가가 아닙니다** |
...
| **GitHub CI (푸시된 모든 커밋)** | push `814f29b` … `9d08c8f` → Actions | 현재 팁(마지막으로 확인한 `b1f054b`)에서 **16/16 job success**입니다(Windows Debug·Release·no-UI 3잡, Core ubuntu·windows × Debug·Release 4잡, macOS — 워크플로마다 PR용·푸시용 2회 실행되어 총 6개 워크플로). 이번에는 **자격 증명으로 잡 로그까지 읽어 단계별 자기 보고를 확인**했습니다: `windows-cli` 잡에서 CLI 검사(33 거부 사례)와 **GUI 컨트롤 목록·슬라이더 경로는 VERIFIED**, **새로 고침 선택 유지·선택 저장/복원 두 단계는 러너에 엔드포인트가 없어 스스로 `SKIPPED`**(실패 아님, 통과로도 세지 않음)로 보고했습니다. `2d0517c`, `a0e1ccb`, `8de1275`, `66468b9`, `8abb973`, `a93c1b1`, `c62acfc`, `e858876`, `b1f054b`도 16/16 — 이 브랜치의 푸시된 커밋은 모두 green입니다(마지막으로 확인한 팁 `b1f054b`) |
...
1. **후속 Draft PR: #25** — <https://github.com/Gomtanga/lowend-circuit/pull/25>
   (base `feature/windows-port` `400fa25`, head `feature/windows-virtual-routing`, draft, 이 브랜치의 모든 커밋에서 CI green). 이 문서를 마지막으로 갱신한 commit의 **부모가 `b1f054b`**(CI 16/16)이고 문서 수정 자체가 그 뒤 커밋입니다. PR #24가 병합되면 base를 `main`으로 바꿉니다.
   **병합은 하지 않았습니다.**
