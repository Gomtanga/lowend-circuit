...
The native app also has a `Spatial Stage` panel. Drag the blue listener point to move the listener in real time, or enter exact values:
...
- `원위치`: resets `나 X` and `나 Z` to the centered listener position.
